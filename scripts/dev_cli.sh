...
ensure_latest_ctr() {
	if [ "$CTR_IMAGE_VERSION" = "local" ]; then
		...
		if [ -n "$CTR_PLATFORM" ]; then
			if ! $DOCKER_RUNTIME pull --platform "$CTR_PLATFORM" "$CTR_IMAGE"; then
				build_container
			fi
		else
...
cmd_help() {
	...
	echo ""
	echo "    build [--debug|--release] [--libc musl|gnu] [--arch x86_64|aarch64] [-- [<cargo args>]]"
	...
	echo "        --arch                Target architecture. Default is the host architecture."
	echo "        --volumes             Hash separated volumes to be exported. Example --volumes /mnt:/mnt#/myvol:/myvol"
	echo "        --hypervisor          Underlying hypervisor. Options kvm, mshv"
	echo ""
	echo "    tests [<test type (see below)>] [--libc musl|gnu] [--arch x86_64|aarch64] [-- [<test scripts args>] [-- [<test binary args>]]] "
	...
	echo ""
	echo "    build-container [--arch x86_64|aarch64]"
...
cmd_build() {
	...
	while [ $# -gt 0 ]; do
		...
		"--arch")
			...
			arch="$(canonical_arch "$1")" ||
				die "Invalid architecture: $1. Valid options are \"x86_64\" and \"aarch64\"."
			;;
		"--volumes")
			shift
			arg_vols="$1"
	...
	ensure_build_dir
	CTR_PLATFORM="$(docker_platform "$arch")"
	ensure_latest_ctr
	...
	[ -e "$exported_device" ] && exported_device_volume="--volume $exported_device"
	...
	$DOCKER_RUNTIME run \
		...
		--workdir "$CTR_CLH_ROOT_DIR" \
		--rm \
		${exported_device_volume:+$exported_device_volume} \
		--volume "$CLH_ROOT_DIR:$CTR_CLH_ROOT_DIR" \
		${exported_volumes:+$exported_volumes} \
...
cmd_tests() {
	...
	if [ "$integration" = true ]; then
		...
		$DOCKER_RUNTIME run \
			...
			--env LLVM_PROFILE_FILE="$LLVM_PROFILE_FILE" \
			"$CTR_IMAGE" \
			dbus-run-session ./scripts/run_integration_tests_"$arch".sh "$@" || fix_dir_perms $? || exit $?
	fi
	...
	if [ "$integration_windows" = true ]; then
		...
		$DOCKER_RUNTIME run \
			...
			--env AUTH_DOWNLOAD_TOKEN="$AUTH_DOWNLOAD_TOKEN" \
			"$CTR_IMAGE" \
			./scripts/run_integration_tests_windows_"$arch".sh "$@" || fix_dir_perms $? || exit $?
	fi
