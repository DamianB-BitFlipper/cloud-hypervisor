...
fn probe_block_device_write_zeroes_support(rdev: libc::dev_t, sysfs_root: &Path) -> bool {
    ...
    let value = match fs::read_to_string(&queue_limit_path) {
        ...
        Err(err) => {
            info!(
                "Block device write zeroes probe could not read {:?}: {}",
                queue_limit_path, err
            );
            return false;
        }
    };
    ...
    let limit = match value.trim().parse::<u64>() {
        ...
        Err(err) => {
            warn!(
                "Block device write zeroes probe could not parse {:?}: {}",
                queue_limit_path, err
            );
            return false;
        }
    ...
    info!(
        "Block device write zeroes probe: path={:?}, write_zeroes_max_bytes={} => {}",
        queue_limit_path, limit, supported
    );
    supported
}
